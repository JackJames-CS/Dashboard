...
import { calendarPageEvents } from '../data/mockData'
...
export default function Calendar() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {view === 'day' && (
        <Card title="Day view — Thursday, Mar 5">
          <div className="space-y-2">
            {calendarPageEvents.filter((e) => e.day === 5).map((ev) => (
              <div
                key={ev.id}
      ...
      {view === 'month' && (
        <Card title="Month view — March 2025">
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {/* March 2025 starts on Saturday — 6 empty cells then 1-31 */}
            {Array.from({ length: 6 }, (_, i) => (
              <div key={`empty-${i}`} className="rounded-lg p-2 min-h-[60px] bg-transparent" />
            ))}
            {Array.from({ length: 31 }, (_, i) => i + 1).map((date) => (
              <div
                key={date}
                className={`rounded-lg p-2 min-h-[60px] text-sm ${
                  date === 5 ? 'ring-1 ring-accent-indigo bg-accent-indigo/5' : 'bg-surface-50'
                }`}
              >
                {date}
              </div>
